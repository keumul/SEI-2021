...
int DecimicalNotation(std::string input, int scaleofnot) {
	std::string val = input.substr(1, input.size());
	return std::stoi(val, nullptr, scaleofnot);
}
namespace Lexer
{
	Graph graphs[N_GRAPHS] =
	{
		...
		{ LEX_LITERAL, FST::FST(GRAPH_INT_UNIQ) },
		{ LEX_NEW, FST::FST(GRAPH_NEW) },
		{ LEX_MAIN, FST::FST(GRAPH_MAIN) },
		{ LEX_ID_TYPE, FST::FST(GRAPH_ENTIER) },
	...
	{
		static int literalValuer = 1;
		char* buf = new char[SCOPED_ID_MAXSIZE], val[3];
		strcpy_s(buf, MAXSIZE_ID, "LTRL");
		_itoa_s(literalValuer++, val, 10);
		strcat_s(buf,10, val);
		return buf;
	}
			...
			for (int j = 0; j < N_GRAPHS; j++)
			{
				...
				if (FST::execute(fst))
				{
					...
					case LEX_LITERAL_NUMO:
					{
						...
						if (value < VAL_MINSIZE || value > VAL_MAXSIZE)
